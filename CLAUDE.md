...
- `extra_body`: deep-merged via `_deep_merge` from `request.extra_body` (client) then `tier_cfg["extra_body"]` (already global+bucket+tier-merged). Config wins per leaf; unrelated client keys are preserved. Keys are lifted to top-level kwargs on the upstream call (works for any OpenAI-compatible backend: llama-server, ollama, vLLM, llama.cpp). Merged keys also land in `litellm_request["extra_body"]["allowed_openai_params"]` so cascade proxies (litellm, OpenAI-SDK) forward them rather than filtering against their default allowlist.
