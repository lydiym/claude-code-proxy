...
- **extra_body merge chain**: `[global] → [bucket] → [tier]` (haiku → `small`, others → `big`). Each layer deep-merges; later wins per leaf. Keys are lifted to top-level kwargs on the upstream call and forwarded to any cascade proxy (litellm, OpenAI-SDK) via `extra_body.allowed_openai_params` so vendor-specific keys (`chat_template_kwargs`, `cache_prompt`, `n_predict`, `reasoning_effort`, …) are not silently dropped.
