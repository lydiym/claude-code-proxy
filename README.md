...
- **extra_body merge chain**: `[global] → [bucket] → [tier]` (haiku → `small`, others → `big`). Each layer deep-merges; later wins per leaf. Keys are lifted to top-level kwargs on the upstream call. The merged key list is also published as `allowed_openai_params` (top-level + inside `extra_body`) so our own litellm hop and any cascade proxy (litellm, OpenWebUI) forward vendor-specific keys (`chat_template_kwargs`, `cache_prompt`, `n_predict`, `reasoning_effort`, …) instead of silently dropping them.
